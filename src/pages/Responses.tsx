...
const Responses: React.FC = () => {
  ...
  const getResponseLabel = (response: string) => {
    switch (response) {
      ...
      case 'P': return 'Parcial/Induzido';
      case 'N': return 'Incapaz';
      case 'X': return 'Não se Aplica';
  ...
  const calculateStats = () => {
    ...
    const allResponseValues = filteredResponses.flatMap(r => Object.values(r.responses));
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-600">Visualize e analise as respostas dos formulários</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white p-6 rounded-lg border border-slate-200">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-slate-600">Total de Respostas</p>
        ...
        {Object.entries(stats.responseDistribution).map(([response, count]) => (
          <div key={response} className="bg-white p-6 rounded-lg border border-slate-200">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-slate-600">{getResponseLabel(response)}</p>
                <p className="text-2xl font-bold text-slate-800">{count}</p>
              </div>
              <div className={`w-8 h-8 rounded-full ${getResponseColor(response)}`}></div>
            </div>
          </div>
        ))}
      </div>
            ...
            <tbody className="bg-white divide-y divide-slate-200">
              {filteredResponses.map((response) => (
                <tr key={response.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-slate-900">{response.formName}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center space-x-1">
                      {Object.values(response.responses).map((resp, index) => (
                        <div
                          key={index}
                          className={`w-6 h-6 rounded-full ${getResponseColor(resp)} flex items-center justify-center text-white text-xs font-bold`}
        ...
        <div className="bg-white rounded-lg border border-slate-200 p-12 text-center">
          ...
          <p className="text-slate-500">
            {searchTerm || selectedForm !== 'all' 
              ...
              : 'As respostas aparecerão aqui quando os formulários forem respondidos'
            }
          </p>
        </div>
      )}
